function Bezier() constructor {
    ...
    __sections_total_length 	= [];	/// @is {number[]}
    __sections_length			= [];	/// @is {number[]}
    ...
    static refresh = function()/*->void*/ {
		...
		for (var i = 0, size_i = array_length(__sub_points); i < size_i - 1; i++) {
			var point		= __sub_points[i];
			var next_point	= __sub_points[i + 1];
			var distance	= point_distance(point.x, point.y, next_point.x, next_point.y);
			total_distance += distance;
			
			__sections_length[i]		= distance; 
			__sections_total_length[i]	= total_distance; 
		}
		
		for (var i = 0, size_i = array_length(__sub_points); i < size_i - 1; i++) {
			__sections_length[i]		/= total_distance;
			__sections_total_length[i]	/= total_distance;
		}
    }
    ...
    static get_point = function(_position/*:number*/)/*->Point*/ {
        var f_position  		= _position * array_length(__sections_total_length);
        var section_index   	= floor(min(array_length(__sections_total_length) - 1, f_position));
        var section_position	= (f_position - section_index);
 
        var is_first_section	= (section_index == 0);
        var is_last_section 	= (section_index == (array_length(__sections_length) - 1));
        if (is_first_section) {
        	...
        	var number_of_sub_points = array_length(__sub_points);
